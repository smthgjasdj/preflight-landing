...
export async function GET(request: NextRequest) {
  ...
  if (!refId || !version || !from) {
    return NextResponse.json({ error: "refId, version and from are required" }, { status: 400 });
  }

  const backend = await fetch(
    `${process.env.API_URL}/m/update?${new URLSearchParams({ uuid: refId }).toString()}`,
    {
      method: "POST",
