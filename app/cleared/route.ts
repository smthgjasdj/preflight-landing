...
export async function GET(request: NextRequest) {
  ...
  if (!refId || !version) {
    return NextResponse.json({ error: "refId and version are required" }, { status: 400 });
  }
  ...
  const backend = await fetch(
    `${process.env.API_URL}/m/install?${new URLSearchParams({ uuid: refId, extension: code }).toString()}`,
    {
      method: "POST",
